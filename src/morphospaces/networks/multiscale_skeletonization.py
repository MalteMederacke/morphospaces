...
from morphospaces.losses.cbdice_loss import SoftcbDiceLoss
...
class MultiscaleSkeletonizationNet_cbdice(pl.LightningModule):
    def __init__(
        self,
        in_channels: int = 1,
        image_key: str = "image",
        ...
        self.scale_0_loss = SoftcbDiceLoss(iter_=30)
        self.scale_1_loss = SoftcbDiceLoss(iter_=30)
        self.scale_2_loss = SoftcbDiceLoss(iter_=30)
    ...
    def _compute_loss(
        ...
        # scale 0 loss
        scale_0_loss = self.scale_0_loss(
            y_pred=skeleton_prediction,
            y_true=skeletonization_target_scale_0,        )

        # scale 1 loss
        scale_1_loss = self.scale_1_loss(
            y_pred=decoder_outputs[1],
            y_true=skeletonization_target_scale_1,
        )

        # scale 2 loss
        scale_2_loss = self.scale_2_loss(
            y_pred=decoder_outputs[0],
            y_true=skeletonization_target_scale_2,
        )
...
class MultiscaleSkeletonizationNet_cbdice_ce(pl.LightningModule):
    def __init__(
        self,
        in_channels: int = 1,
        image_key: str = "image",
        ...
        scale_1_loss_coefficient: float = 0.3,
        scale_2_loss_coefficient: float = 0.1,
        relative_loss1_coefficient: float = 1, 
        lr_scheduler_step: int = 1000,
        lr_reduction_factor: float = 0.2,
        lr_reduction_patience: int = 15,
    ):
        ...
        self.scale_0_loss1 = WeightedCrossEntropyLoss()
        self.scale_1_loss1 = WeightedCrossEntropyLoss()
        self.scale_2_loss1 = WeightedCrossEntropyLoss()
    ...
    def _compute_loss(
        ...
        scale_0_loss1 = self.scale_0_loss1(
            input=skeleton_prediction,
            target=labels_scale_0.squeeze(dim=1).long(),
        )
        # scale 1 loss
        scale_1_loss = self.scale_1_loss(
            y_pred=decoder_outputs[1],
        ...
        scale_1_loss1 = self.scale_1_loss1(
            input=decoder_outputs[1],
            target=labels_scale_1.squeeze(dim=1).long(),
        )
        ...
        scale_2_loss1 = self.scale_2_loss1(
            input=decoder_outputs[0],
            target=labels_scale_2.squeeze(dim=1).long(),
        )
        coeff = self.hparams.relative_loss1_coefficient

        return (
            (self.hparams.scale_0_loss_coefficient * scale_0_loss) + (
                self.hparams.scale_1_loss_coefficient * coeff * scale_0_loss1)
            + (self.hparams.scale_1_loss_coefficient * scale_1_loss) + (
                self.hparams.scale_1_loss_coefficient * coeff * scale_1_loss1)
            + (self.hparams.scale_2_loss_coefficient * scale_2_loss) + (
                self.hparams.scale_2_loss_coefficient * coeff * scale_2_loss1)
        )




class MultiscaleSkeletonizationNet_cbdice_L1(pl.LightningModule):
    def __init__(
        self,
        in_channels: int = 1,
        image_key: str = "image",
        ...
        self.scale_0_loss1 = MaskedSmoothL1Loss(reduction="mean")
        self.scale_1_loss1 = MaskedSmoothL1Loss(reduction="mean")
        self.scale_2_loss1 = MaskedSmoothL1Loss(reduction="mean")
