...
class MultiscaleSemanticSegmentationNet_cbdice(pl.LightningModule):
    def __init__(
        self,
        in_channels: int = 1,
        out_channels: int = 2,
    ...
    def _compute_loss(
        ...
        scale_0_loss = self.scale_0_loss(
            y_pred=skeleton_prediction,                  # <- raw logits
            y_true=labels_scale_0                         # <- raw labels
        )

        # Scale 1 loss
        scale_1_loss = self.scale_1_loss(
            y_pred=decoder_outputs[1],                    # <- raw logits
            y_true=labels_scale_1                         # <- raw labels
        )

        # Scale 2 loss
        scale_2_loss = self.scale_2_loss(
            y_pred=decoder_outputs[0],                    # <- raw logits
            y_true=labels_scale_2                         # <- raw labels
        )
